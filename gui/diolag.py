...
class LoginDiolag(wx.Dialog):
    ...
    def __init__(self, parent):
        super(LoginDiolag, self).__init__(parent, title=u"登录", size=(350, 300))
        panel = wx.Panel(self)
        ...
        name_label = Text(panel, u"用户名", size=(75, 50), style=wx.TE_LEFT, font=18)
        self.name = wx.TextCtrl(panel, wx.ID_ANY, size=(200, 50))
        name_sizer.Add(name_label, 0, wx.EXPAND | wx.TOP | wx.LEFT, 25)
        name_sizer.Add(self.name, 0, wx.EXPAND | wx.TOP | wx.LEFT, 25)

        password_label = Text(panel, u"密   码", size=(75, 50), style=wx.TE_LEFT, font=18)
        self.password = wx.TextCtrl(panel, wx.ID_ANY, size=(200, 50), style=wx.TE_PASSWORD)
...
class SelectDiolag(wx.Dialog):
    ...
    def __init__(self, parent, title, express_list):
        ...
        express_name.insert(0, u"--请选择快递公司--")
        ...
        self.batch = wx.ComboBox(panel, choices=[u'--请选择批次--'], style=wx.CB_DROPDOWN)
    ...
    def add_batch(self, event):
        ...
        if body:
            ...
            if dlg.ShowModal() == wx.ID_YES:
                ...
                server.selected['batch_id'] = body['id']
